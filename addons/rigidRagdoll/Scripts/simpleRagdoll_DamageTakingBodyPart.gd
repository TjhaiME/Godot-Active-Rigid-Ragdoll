...
func _ready():
	...
	#method 1 #need to also set contact monitor and collisions reported
...
func my_body_entered(body):
	return
	my_body_entered_func(body)

func my_body_entered_func(body):
	#print("body entered")
...
func generate_detection_area(): 
	#seems to be better for detecting damage in mass army formations,
	# as I can use collision layers instead of exceptions AND I dont have to 
	#turn on contact monitor and collisions reported on the bodies 
	#(which needs to be high as they always intersect with the other body parts)
	#also leads to more stable damage detection but less flexible if I use a different collision layer, fine for minigames
	
	var areaNode = Area.new()
	add_child(areaNode)
	#collisionShape
	var colRef = get_node_or_null("CollisionShape")
	if colRef == null:
		print("couldnt find collision child to make a detection area with for node ", self)
		return false
	var colDupe = colRef.duplicate()
	areaNode.add_child(colDupe)
	#transforms
	areaNode.global_transform = global_transform
	colDupe.transform = colRef.transform
	#collision layers (layer 5) random choice
	areaNode.collision_layer = 16
	areaNode.collision_mask = 16
	#signal
	areaNode.connect("body_entered", self, "detectedBody")


func detectedBody(body):
	my_body_entered_func(body)
